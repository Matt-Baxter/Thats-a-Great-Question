...
- [ ] T002 [P] Create `.env.example` containing only the line `QUESTION_APP_API_KEY=` and a comment that the real value goes in `.env` locally and in Vercel's environment settings — never in the repository — and that the app deliberately does not read `ANTHROPIC_API_KEY` (research.md R11)
...
- [ ] T004 (maintainer) Settle the four items in research.md "Must verify before implementation" — the maximum function duration on Vercel Hobby, firewall rate limiting on Hobby, whether Vercel's Python runtime accepts a `BaseHTTPRequestHandler` subclass and bundles a root-level package, and where the Anthropic spend limit is set — and record each answer in research.md. If a root-level package is not bundled, every later task uses `api/_inquiry/` in place of `inquiry/`. **Open:** Phases 1–3 were built on the default assumption for each item, recorded in research.md on 2026-09-25; verify before T047
...
- [ ] T025 [US1] Run `pytest` until everything passes. Then, with `QUESTION_APP_API_KEY` set, run `vercel dev` and work through quickstart.md scenarios 1, 9 and 10
